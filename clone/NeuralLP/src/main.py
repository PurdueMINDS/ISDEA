...
def main():
    ...
    parser.add_argument("--no_link_percent", default=0.0, type=float)
    ...
    parser.add_argument("--resume", required=True, type=str)  # MODIFY
    ...
    if len(option.resume) > 0:  # MODIFY
        # # MODIFY
        with open(
            os.path.join(option.resume, "ckpt", "checkpoint"), "r"
        ) as file:  # MODIFY
            # # MODIFY
            for line in file:  # MODIFY
                # # MODIFY
                _, resume_epoch, _ = line.split('"')  # MODIFY
                break  # MODIFY
        option.from_model_ckpt = os.path.join(  # MODIFY
            option.resume, "ckpt", resume_epoch  # MODIFY
        )  # MODIFY
    with tf.Session(config=config) as sess:
        ...
        if len(option.resume) == 0 and not option.no_train:  # MODIFY
