...
class Data(object):
    def __init__(self, folder, seed, type_check, domain_size, no_extra_facts):
        ...
        self.relation_file = os.path.join(folder, "relations.dict")  # MODIFY
        self.entity_file = os.path.join(folder, "entities.dict")  # MODIFY
        ...
        is_trans = os.path.basename(folder).split("-")[-1] == "trans"  # MODIFY
        is_ind = os.path.basename(folder).split("-")[-1] == "ind"  # MODIFY
        assert is_trans or is_ind  # MODIFY
        words = os.path.basename(folder).split("-")  # MODIFY
        assert len(words) == 2 or (len(words) == 3 and words[1] == "perm")
        title = words[0]
        ...
        if is_trans:  # MODIFY
            ...
            if title in ["FD1", "FD2"]:
                #
                self.facts_file = os.path.join(folder, "observe.txt")  # MODIFY
                self.share_db = True  # MODIFY
            else:
                #
                self.facts_file = os.path.join(folder, "train.txt")  # MODIFY
                self.share_db = True  # MODIFY
            self.train_file = os.path.join(folder, "train.txt")  # MODIFY
            self.valid_file = os.path.join(folder, "valid.txt")  # MODIFY
            self.test_file = os.path.join(folder, "valid.txt")  # MODIFY
        else:  # MODIFY
            # # MODIFY
            self.facts_file = os.path.join(folder, "observe.txt")  # MODIFY
            self.share_db = True  # MODIFY
            self.train_file = os.path.join(folder, "observe.txt")  # MODIFY
            self.valid_file = os.path.join(folder, "test.txt")  # MODIFY
            self.test_file = os.path.join(folder, "test.txt")  # MODIFY
        ...
        if self.share_db:
            ...
            self.train_facts, self.num_train_fact = self._parse_triplets(
                self.train_facts_file
            )
            self.test_facts, self.num_test_fact = self._parse_triplets(
                self.test_facts_file
            )
        ...
        self.query_for_rules = list(
            set(zip(*self.train)[0])
            | set(zip(*self.test)[0])
            | set(zip(*self._augment_with_reverse(self.train))[0])
            | set(zip(*self._augment_with_reverse(self.test))[0])
        )
    ...
    def _create_parser(self):
        ...
        assert (
            self.num_query == 2 * len(self.relation_to_number) == 2 * self.num_relation
        )
        parser = {"query": {}, "operator": {}}
        number_to_relation = {
            value: key for key, value in self.relation_to_number.items()
        }
    ...
    def _parse_domains_file(self, file_name):
        ...
        with open(file_name, "r") as f:
            for line in f:
                ...
                if len(this_domain) == self.domain_size:
                    ...
                    remains = [
                        i for i in xrange(self.num_relation) if i not in this_domain
                    ]
        ...
        for r in xrange(self.num_relation):
            if r not in result.keys():
                result[r] = np.random.choice(
                    range(self.num_relation), self.domain_size, replace=False
                )
        return result
    ...
    def _numerical_encode(self):
        ...
        with open(self.relation_file) as f:
            for line in f:
                ...
                relation_to_number[msg_name] = int(msg_id)  # MODIFY
        ...
        with open(self.entity_file) as f:
            for line in f:
                ...
                msg_id, msg_name = re.split(r"\s+", line.strip())  # MODIFY
                entity_to_number[msg_name] = int(msg_id)  # MODIFY
    ...
    def _parse_triplets(self, file):
        ...
        with open(file) as f:
            for line in f:
                # \\:l = line.strip().split("\t")
                l = re.split(r"\s+", line.strip())  # MODIFY
                assert len(l) == 3
                output.append(
                    (
                        self.relation_to_number[l[1]],
    ...
    def _db_to_matrix_db(self, db):
        matrix_db = {
            r: ([[0, 0]], [0.0], [self.num_entity, self.num_entity])
            for r in xrange(self.num_relation)
        }
    ...
    def _count_batch(self, samples, batch_size):
        ...
        num_batches = [ceil(1.0 * x / batch_size) for x in relations_counts.values()]
    ...
    def _augment_with_reverse(self, triplets):
        ...
        for triplet in triplets:
            augmented += [
                triplet,
                (triplet[0] + self.num_relation, triplet[2], triplet[1]),
            ]
        return augmented
    ...
    def _next_batch(self, start, size, samples):
        ...
        if self.type_check:
            ...
            batch_size = next(
                (
                    i
                    for i in range(len(this_batch_tmp))
    ...
    def next_train(self):
        ...
        if self.share_db and self.use_extra_facts:
            extra_facts = [
                fact for i, fact in enumerate(self.train) if i not in this_batch_id
            ]
...
class DataPlus(Data):
    def __init__(self, folder, seed):
        ...
        self.all_exams = set(
            [tuple(q + [h, t]) for (q, h, t) in self.train + self.test + self.valid]
        )
        ...
        self.query_for_rules = [
            list(q) for q in set([tuple(q) for (q, _, _) in self.test + self.train])
        ]
    ...
    def _parse_examples(self, file_name):
        ...
        triplets = [
            [
                [self.query_vocab_to_number[w] for w in l[1].split(",")],
    ...
    def _parse_facts(self, file_name):
        ...
        facts = [
            [
                self.kb_relation_to_number[l[1]],
                self.kb_entity_to_number[l[0]],
                self.kb_entity_to_number[l[2]],
            ]
            for l in lines
    ...
    def _create_parser(self):
        parser = {"operator": {}}
        number_to_relation = {
            value: key for key, value in self.kb_relation_to_number.items()
        }
        number_to_query_vocab = {
            value: key for key, value in self.query_vocab_to_number.items()
        }

        parser["query"] = (
            lambda ws: ",".join([number_to_query_vocab[w] for w in ws]) + " "
        )
