...
def links2subgraphs(A, graphs, params, max_label_value=None):
    ...
    def extraction_helper(A, links, g_labels, split_env):
        ...
        # \\:    )
        with open(os.path.join(split_env, "num_graphs"), "wb") as file:  # MODIFY
            pickle.dump(len(links), file)  # MODIFY
        ...
        with mp.Pool(
            ...
            for (str_id, datum) in tqdm(
                ...
                serialize_path(
                    datum, os.path.join(split_env, "cache", str_id)
                )  # MODIFY
    ...
    for split_name, split in graphs.items():
        ...
        split_env = os.path.join(env, db_name_pos)  # MODIFY
        os.makedirs(split_env, exist_ok=True)  # MODIFY
        os.makedirs(os.path.join(split_env, "cache"), exist_ok=True)  # MODIFY
        ...
        split_env = os.path.join(env, db_name_neg)  # MODIFY
        os.makedirs(split_env, exist_ok=True)  # MODIFY
        os.makedirs(os.path.join(split_env, "cache"), exist_ok=True)  # MODIFY
    ...
    # \\:        )
    for (str_id, datum) in [  # MODIFY
        ("max_n_label_sub", int(max_n_label["value"][0])),  # MODIFY
        ("max_n_label_obj", int(max_n_label["value"][1])),  # MODIFY
        ("avg_subgraph_size", float(np.mean(subgraph_sizes))),  # MODIFY
        ("min_subgraph_size", float(np.min(subgraph_sizes))),  # MODIFY
        ("max_subgraph_size", float(np.max(subgraph_sizes))),  # MODIFY
        ("std_subgraph_size", float(np.std(subgraph_sizes))),  # MODIFY
        ("avg_enc_ratio", float(np.mean(enc_ratios))),  # MODIFY
        ("min_enc_ratio", float(np.min(enc_ratios))),  # MODIFY
        ("max_enc_ratio", float(np.max(enc_ratios))),  # MODIFY
        ("std_enc_ratio", float(np.std(enc_ratios))),  # MODIFY
        ("avg_num_pruned_nodes", float(np.mean(num_pruned_nodes))),  # MODIFY
        ("min_num_pruned_nodes", float(np.min(num_pruned_nodes))),  # MODIFY
        ("max_num_pruned_nodes", float(np.max(num_pruned_nodes))),  # MODIFY
        ("std_num_pruned_nodes", float(np.std(num_pruned_nodes))),  # MODIFY
    ]:  # MODIFY
        # # MODIFY
        with open(os.path.join(env, str_id), "wb") as file:  # MODIFY
            pickle.dump(datum, file)  # MODIFY
