...
def process_files(files, saved_relation2id, add_traspose_rels):
    ...
    relation2id = {}  # MODIFY
    with open(  # MODIFY
        os.path.join(os.path.dirname(files["links"]), "entities.dict"), "r"  # MODIFY
    ) as file:  # MODIFY
        # # MODIFY
        for line in file:  # MODIFY
            # # MODIFY
            msg_id, msg_name = re.split(r"\s+", line.strip())  # MODIFY
            entity2id[msg_name] = int(msg_id)  # MODIFY
    with open(  # MODIFY
        os.path.join(os.path.dirname(files["links"]), "relations.dict"),
        "r",  # MODIFY
    ) as file:  # MODIFY
        # # MODIFY
        for line in file:  # MODIFY
            # # MODIFY
            msg_id, msg_name = re.split(r"\s+", line.strip())  # MODIFY
            relation2id[msg_name] = int(msg_id)  # MODIFY
    if "-perm-" in files["links"]:  # MODIFY
        #
        relation2id = {
            msg_name: len(relation2id) - 1 - msg_id
            for (msg_name, msg_id) in relation2id.items()
        }  # MODIFY
    for key in saved_relation2id:  # MODIFY
        # # MODIFY
        del saved_relation2id[key]  # MODIFY
    saved_relation2id = None  # MODIFY
    ...
    for i in range(len(relation2id)):  # MODIFY
...
def ssp_multigraph_to_dgl(graph, n_feats=None):
    ...
    g_dgl = dgl.from_networkx(g_nx, edge_attrs=["type"])  # MODIFY
    # add node features
...
def get_subgraphs(
    ...
    for link in all_links:
        ...
        subgraph = dgl_adj_list.subgraph(nodes)  # MODIFY
        # \\:subgraph.edata["type"] = dgl_adj_list.edata["type"][
        # \\:    dgl_adj_list.subgraph(nodes).parent_eid
        # \\:]
        ...
        if not subgraph.has_edges_between(0, 1):  # MODIFY
            subgraph.add_edges(0, 1)  # MODIFY
            subgraph.edata["type"][-1] = torch.tensor(rel).type(
                torch.LongTensor
            )  # MODIFY
            subgraph.edata["label"][-1] = torch.tensor(rel).type(
                torch.LongTensor
            )  # MODIFY
...
def main(params):
    ...
    # \\:)
    logger.info(  # MODIFY
        f"MRR | Hits@1 | Hits@3 | Hits@5 | Hits@10 : {mrr} | {hits_1} | {hits_3} | {hits_5} | {hits_10}"  # MODIFY
    )  # MODIFY
...
if __name__ == "__main__":
    ...
    parser.add_argument(  # MODIFY
        "--resume",  # MODIFY
        type=str,  # MODIFY
        required=True,  # MODIFY
        help="Resuming experiments",  # MODIFY
    )  # MODIFY
    ...
    # \\:}
    params.file_paths = {  # MODIFY
        "graph": os.path.join("./data", params.dataset, "observe.txt"),  # MODIFY
        "links": os.path.join("./data", params.dataset, "test.txt"),  # MODIFY
    }  # MODIFY
    ...
    # \\:)
    params.model_path = os.path.join(
        params.resume, "best_graph_classifier.pth"
    )  # MODIFY
    ...
    # \\:)
    os.makedirs(
        os.path.join("experiments", params.experiment_name), exist_ok=True
    )  # MODIFY
    file_handler = logging.FileHandler(  # MODIFY
        os.path.join(
            "experiments", params.experiment_name, f"log_rank_test.txt"
        )  # MODIFY
    )  # MODIFY
