...
def process_files(files, saved_relation2id=None):
    ...
    relation2id = {}  # MODIFY
    with open(  # MODIFY
        os.path.join(os.path.dirname(files["valid"]), "entities.dict"), "r"  # MODIFY
    ) as file:  # MODIFY
        # # MODIFY
    ...
    with open(  # MODIFY
        os.path.join(os.path.dirname(files["valid"]), "relations.dict"),
        "r",  # MODIFY
    ) as file:  # MODIFY
        # # MODIFY
        for line in file:  # MODIFY
    ...
    if "train0" in triplets:  # MODIFY
        # # MODIFY
        triplets["train"] = np.concatenate(
            (triplets["train0"], triplets["train1"])
        )  # MODIFY
        del triplets["train0"]  # MODIFY
        del triplets["train1"]  # MODIFY
