...
def serialize_path(data, path):  # MODIFY
    data_tuple = tuple(data.values())  # MODIFY
    with open(path, "wb") as file:  # MODIFY
        pickle.dump(data_tuple, file)  # MODIFY
...
def deserialize_path(path):  # MODIFY
    with open(path, "rb") as file:  # MODIFY
        data_tuple = pickle.load(file)  # MODIFY
    keys = ("nodes", "r_label", "g_label", "n_label")  # MODIFY
    return dict(zip(keys, data_tuple))  # MODIFY
...
def ssp_to_torch(A, device, dense=False):
    ...
    A = torch.sparse.FloatTensor(idx, dat, torch.Size([A.shape[0], A.shape[1]])).to(
        device=device
    )
    return A
