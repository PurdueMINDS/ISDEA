...
def evaluate():
    ...
    print("Mean rank %0.2f" % (1.0 * ranks / line_cnt))
    print("Mean Reciprocal Rank %0.4f" % (1.0 * rranks / line_cnt))

    if option.v:
        hits_by_q_mean = sorted(
            [[k, np.mean(v), len(v)] for k, v in hits_by_q.items()],
            key=lambda xs: xs[1],
            reverse=True,
        )
